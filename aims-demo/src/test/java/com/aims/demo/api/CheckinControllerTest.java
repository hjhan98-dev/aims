...
class CheckinControllerTest {
    ...
    @Test
    void checkinReturnsConfirmation() {
        CheckinRequest request = new CheckinRequest("OZ1234", "Hong Gildong");

        ResponseEntity<CheckinResponse> response = restTemplate.postForEntity(
                "http://localhost:" + port + "/api/passengers/checkin", request, CheckinResponse.class);
        ...
        assertThat(response.getBody().checkinId()).isNotBlank();
    }
}
